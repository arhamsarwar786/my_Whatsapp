...
var firebaseInstance = FirebaseFirestore.instance.collection("chats").doc(GlobalState.myNumber).collection("personalChats");
 ...
 getUnSeenMessages(uid){
  ...
  var seenTime ;
    ...
    firebaseInstance.doc(uid).snapshots().listen((event) {
      // print(DateTime.parse(event.get('seenTime').toDate().toString()).compareTo(DateTime.now()) < 0 );
      seenTime  = DateTime.parse(event.get('seenTime').toDate().toString());
    firebaseInstance
                .doc(uid)
                .collection("privateChat").get().then((value){
                value.docs.forEach((element) {
                    print("inside me");
                  if(element.get("isSentByMe") == false){
                    // print(element.get("isSentByMe") == false);
                  if (DateTime.parse(element.get("time").toDate().toString()).compareTo(seenTime)  > 0) {
...
fetchContactsAndCheckStatus(){
 ...
 GlobalState.dataListen!.forEach((element) {
      // print(element.id);
        ...
        firebaseInstance.doc(element.id).collection('privateChat').snapshots().listen((event) {
          // print(event.docs);
          ...
          event.docs.forEach((item) { 
            ...
            if(item.get("status") == "sent"){
                // print(item);
                firebaseInstance.doc(element.id).collection('privateChat').doc(item.id).update({"isSeen": false, "deliverTime": DateTime.now(),"status":"deliver"});
                              // .update(
